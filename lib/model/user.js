...
function ProxyModel(mongoose) {
     ...
     * @param query = {username: 'some username'}
     * @returns {adapter.deferred.promise|*|promise|Promise.deferred.promise|Q.promise}
     */
    ProxyUserSchema.statics.findUserWithPromise = function (query) {
        var deferred = Q.defer();
        this.findOne(query, filtered_fields, function (err, user) {
            if (err) {
                deferred.reject(new Error("User '" + query.username + "' not found"));
            } else {
